...
#### 背景
联赛已获得知名主播背书，流量预期从「20 桌内部赛」提升到「大量观众同时在线」。当前架构（Flask 单线程 + 轮询 + werkzeug 内置服务器）在高并发下会成为瓶颈。

#### 当前瓶颈分析

| 组件 | 现状 | 高流量下的问题 |
|------|------|---------------|
| Web 服务器 | Flask 自带 werkzeug（单线程） | 100 并发就卡死 |
| 前端轮询 | 3 个端点，分别 5s / 5s / 5s | 1000 用户 = ~400 请求/秒，大部分是空查询 |
| MongoDB 查询 | 每次请求实时聚合 | 高频聚合吃 CPU，排行榜等可以缓存 |
| Session | Flask 默认服务端 session（内存） | 多进程不共享，负载均衡下登录状态丢失 |

#### 改造方案（优先级排序）

**Phase 1 — 最小改动，立即见效**

1. **换 WSGI 服务器**：Flask + gunicorn + gevent
   - gevent 给每个请求一个 greenlet，I/O 等待时不阻塞
   - 改动量：新增 `gunicorn.conf.py`，部署命令从 `python app.py` 改为 `gunicorn -c gunicorn.conf.py app:app`
   - 能扛 500-1000 并发
   - ⚠️ 需要确保 PyMongo 连接池配置正确（`maxPoolSize`）

2. **排行榜缓存**：排行榜数据变化频率极低（一局结束才变），加 30 秒缓存
   - 聚合管道结果缓存在内存，30 秒内直接返回
   - 减少 95%+ 的聚合查询

**Phase 2 — 体验升级**

3. **轮询改 SSE**（Server-Sent Events）
   - 进行中对局、报名队列、等待队列改为服务端推送
   - 有变更时毫秒级通知，没变更时零流量
   - 需要 gevent 配合（长连接支持）
   - Flask 原生支持 SSE（`Response` + generator），不需要额外依赖

4. **静态资源 CDN**：Tailwind CSS CDN → 打包成静态文件上 CDN
   - 减少对服务器的静态文件请求

**Phase 3 — 长期架构**

5. **考虑迁移到 Quart**（Flask 的 async 版本）
   - API 几乎完全兼容 Flask，改动量小
   - 原生 async 支持，天然适合 SSE + 高并发
   - 时机：当 Phase 1-2 都做完仍然不够时

#### 关键决策点
- Phase 1 应在比赛前完成（gunicorn + 缓存）
- Phase 2 根据实际流量决定是否需要
- Phase 3 除非流量到数千人否则不急
...
def sse_active_games():
    def generate():
        ...
        while True:
            ...
            fingerprint = json.dumps([g.get("gameUuid") for g in games])
            if fingerprint != last_fingerprint:
                last_fingerprint = fingerprint
                yield f"data: {json.dumps(games)}\n\n"
            gevent.sleep(1)  # 或用 MongoDB change stream 替代轮询
    return Response(generate(), mimetype="text/event-stream")
```

**前端 SSE 替换轮询：**
```javascript
// 替换 setInterval + fetch
const es = new EventSource('/api/events/active-games');
es.onmessage = (e) => {
