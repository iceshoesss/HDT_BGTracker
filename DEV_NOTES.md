...
### Lo=0 问题调查记录（2026-04-20，待确认根因）

**现象**：HearthMirror `GetBattlegroundsLobbyInfo()` 返回 8 个玩家，总有一个 `AccountId.Lo=0`（index 固定为最后一个，AccountId 对象非 null 但 Hi=0, Lo=0）。

**时间线**（2026-04-19 HDT 插件日志）：
- 15:12 Game 1 → 8 个全部非零 ✅
- 15:18 Game 2 → 7 + 一个 0 ❌
- 15:37 Game 3 → 7 + 一个 0 ❌

Game 1 和 Game 2 之间，bg_parser 的 HearthMirror pythonnet 集成正在开发测试中（commit 15:03-15:20）。

**已排除的假设**：
1. ❌ 时序问题：STEP 13 等 2 秒、换 STEP 9 重读、多次重试均无法解决
2. ❌ bg_tool 并发干扰（待验证）：用户关闭 HDT 后 bg_tool 单独运行仍然 Lo=0
3. ❌ 炉石客户端更新：同一时段内无更新

**待验证假设**：
- bg_tool（pythonnet `new Reflection()`）与 HDT 插件（IPC `Reflection.StartIpcClient()`）同时运行时，可能污染 HearthMirror IPC 服务器状态，导致后续读取永久返回不完整数据
- 验证方法：完全重启炉石 + HDT（确认无 HearthMirror IPC 进程残留），不启动 bg_tool，打一局看是否恢复 8 个非零

**临时方案**：
- 客户端（bg_tool）：过滤 Lo=0（7 人有效数据）
- 服务端（check-league）：容忍缺失 1 个 Lo 的匹配
- 长期方案：bg_tool 不直接调 HearthMirror，改为通过 HTTP 从 HDT 插件获取已缓存的玩家数据
