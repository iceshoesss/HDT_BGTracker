...
HearthMirror 每次获取 8 个玩家时，总有一个玩家的 AccountId.Lo 为 0（AccountId 不为 null 但 Lo=0）。
原因未知，可能是内存读取时序问题。

**影响**：accountIdLoList 包含 0 会导致服务端匹配失败。
**解决方案**：客户端过滤 Lo=0 再发送。服务端匹配改为"非零 Lo 全部匹配即判定为同一局"（容忍缺失 1 个）。
