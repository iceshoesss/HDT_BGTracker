...
> 2026-04-24 解决：用 Lo 集合生成确定性 UUID
...
同一局对局中，每个玩家的插件生成不同的 gameUuid，服务端会创建多条对局记录。

### 解决方案

用 Lo 集合生成确定性 UUID：`sorted(非零Lo列表)` → SHA256 → 取前 16 字节 → UUID 格式。

- 同一局游戏的 8 个 Lo 集合相同 → 生成的 UUID 完全一致
- bg_tool：`ApiClient.GenerateDeterministicUuid(lobbyPlayers)`
- HDT 插件：`CheckLeagueQueue` 中同样算法，存入 `_cachedGameUuid`
- 服务端无需改动，收到的 gameUuid 已经是同一个值
